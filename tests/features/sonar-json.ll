; RUN: rm -f "%t.json"
; RUN: %parcoach -analysis-db="%t.json" %s 2>&1 | %filecheck %s
; RUN: cat "%t.json" | %filecheck %s --check-prefixes=CHECK-ONCE
; RUN: %parcoach -analysis-db="%t.json" %s 2>&1 | %filecheck %s
; Several successive calls enrich the database, it doesn't reset it.
; RUN: cat "%t.json" | %filecheck %s --check-prefixes=CHECK-ONCE,CHECK-TWICE
; Instrument the code but don't run it.
; CHECK: warning: MPI_Reduce line 10 possibly not called by all processes because of conditional(s) line(s)  24
; CHECK-ONCE: "message": "MPI_Reduce may not be called by all MPI processes"
; CHECK-ONCE: "startLine": 10
; CHECK-ONCE: "secondaryLocations": [
; CHECK-ONCE: "message": "because this condition depends on the rank",
; CHECK-ONCE: "startLine": 24
; CHECK-TWICE: "message": "MPI_Reduce may not be called by all MPI processes"
; CHECK-TWICE: "startLine": 10
; CHECK-TWICE: "secondaryLocations": [
; CHECK-TWICE: "message": "because this condition depends on the rank",
; CHECK-TWICE: "startLine": 24
; RUN: echo "foo" > "%t.wrong.json"
; RUN: %parcoach -analysis-db="%t.wrong.json" %s 2>&1 | %filecheck %s --check-prefix=CHECK-ERROR
; CHECK-ERROR: Could not load analysis database
; RUN: chmod 400 "%t.json"
; RUN: %parcoach -analysis-db="%t.json" %s 2>&1 | %filecheck %s --check-prefix=CHECK-ERROR-WRITE
; CHECK-ERROR-WRITE: Could not open file
