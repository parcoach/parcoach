...
void ParcoachAnalysisIntra::run() {
  // Parcoach analysis
  for (Function &F : M) {
    ...
    // (1) Set the sequence of collectives with a BFS from the exit node in the
    // CFG
    errs() << "BFS\n";
    if (optMpiTaint)
      MPI_BFS(&F);
...
void ParcoachAnalysisIntra::setCollSet(BasicBlock *BB) {
...
void ParcoachAnalysisIntra::setMPICollSet(BasicBlock *BB) {
...
// Tag loop preheader
void ParcoachAnalysisIntra::Tag_LoopPreheader(llvm::Loop *L) {
...
void ParcoachAnalysisIntra::MPI_BFS_Loop(llvm::Function *F) {
...
// FOR MPI APPLIS: BFS
void ParcoachAnalysisIntra::MPI_BFS(llvm::Function *F) {
  std::vector<BasicBlock *> Unvisited;
    ...
    for (; PI != E; ++PI) {
      ...
      if (bbVisitedMap[Pred] == white) {
        ...
        if (mpiCollMap[Pred].empty()) {
          ...
          for (auto &pair : mpiCollMap[header]) {
            if (mpiCollMap[Pred][pair.first].empty()) // copy only empty
                                                      // sequences, others
                                                      // should have NAVS
              mpiCollMap[Pred][pair.first] = mpiCollMap[header][pair.first];
          }
...
bool ParcoachAnalysisIntra::mustWait(llvm::BasicBlock *bb) {
...
// BFS ON EACH LOOP
void ParcoachAnalysisIntra::BFS_Loop(llvm::Function *F) {
  std::vector<BasicBlock *> Unvisited;
...
// BFS
void ParcoachAnalysisIntra::BFS(llvm::Function *F) {
  std::vector<BasicBlock *> Unvisited;
...
void ParcoachAnalysisIntra::checkCollectives(llvm::Function *F) {
  MDNode *mdNode;
...
void ParcoachAnalysisIntra::instrumentFunction(llvm::Function *F) {
...
void ParcoachAnalysisIntra::insertCC(llvm::Instruction *I, int OP_color,
...
std::string ParcoachAnalysisIntra::getWarning(llvm::Instruction &inst) {
  string warning = " ";
